...
def get_segment_number_from_header(header):
    """
    Extrae el número de segmento del encabezado de la secuencia
    """
    try:
        # Primero intentar con el formato simple "segment X"
        import re
        match = re.search(r'segment\s*(\d)', header.lower())
        if match:
            return int(match.group(1))
            
        # Si no encuentra, buscar en formato detallado
        if 'segment' in header.lower():
            for i in range(1, 9):
                if f'segment {i}' in header.lower():
                    return i
                    
    except Exception as e:
        print(f"Error al extraer número de segmento: {str(e)}")
    return None

def clean_artificial_header(header):
    """
    Limpia y formatea el encabezado de las secuencias artificiales
    """
    try:
        ...
        # Extraer el nombre de la cepa antes de "segment"
        parts = header.split('segment')
        if len(parts) > 1:
            strain_name = parts[0].strip()
            # Eliminar espacios extra y guiones bajos al final si existen
            strain_name = strain_name.rstrip('_ ')
            return f">{strain_name}"
            
        return f">{header}"
    except Exception as e:
        print(f"Error al limpiar encabezado: {str(e)}")
        return header

def process_artificial_sequences(artificial_file):
    """
    Procesa el archivo de secuencias artificiales y las agrega a los archivos de segmentos correspondientes
    """
    print("\nProcesando secuencias artificiales...")
    
    try:
        sequences_by_segment = {i: [] for i in range(1, 9)}
        current_header = None
        current_sequence = []
        
        with open(artificial_file, 'r') as f:
            for line in f:
                if line.startswith('>'):
                    # Guardar la secuencia anterior si existe
                    if current_header and current_sequence:
                        segment_num = get_segment_number_from_header(current_header)
                        if segment_num:
                            clean_header = clean_artificial_header(current_header)
                            sequences_by_segment[segment_num].append((
                                clean_header,
        ...
        # Escribir las secuencias en los archivos correspondientes
        for segment_num, sequences in sequences_by_segment.items():
            if sequences:
        ...
        print(f"No se encontró el archivo {artificial_file}")
    except Exception as e:
        print(f"Error procesando secuencias artificiales: {str(e)}")
...
def extract_segments_multi(avian_files, human_files, individual_files, avian_samples=13, human_samples=5):
    """
    ...
    for input_file, subtype in avian_files.items():
        ...
        num_samples = min(avian_samples, len(sequences_by_strain))
...
# Procesar archivos humanos
    ...
    for input_file, subtype in human_files.items():
        ...
        num_samples = min(human_samples, len(sequences_by_strain))
        selected_strains = random.sample(list(sequences_by_strain.keys()), num_samples)
    ...
    for segment_num in range(1, 9):
        ...
        try:
            with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
                for line in infile:
                    if line.startswith('>'):
                        # Para secuencias artificiales, mantener el nombre completo
                        if 'variant' in line:
                            # Asegurar que el encabezado esté limpio
                            clean_line = line.strip()
                            if not clean_line.endswith(')'):
                                clean_line = clean_line + ')'
                            outfile.write(f"{clean_line}\n")
                        else:
                            # Para secuencias normales, mantener el procesamiento original
                            start_idx = line.find('(')
                            end_idx = line.find(')') + 1
...
def main():
    # Definir los archivos de entrada originales
    avian_files = {
    ...
    try:
        ...
        # Primero procesar los archivos originales
        extract_segments_multi(avian_files, human_files, individual_files)
        
        # Procesar el archivo de secuencias artificiales
        process_artificial_sequences('artificial_sequences(1).fasta')
